---
name: jobsight
description: JobSight에 저장된 기업 정보·복지, 채용공고·지원 상태·절차, 모집 직무·역량, 직무 참고 정보, 기업 뉴스·유튜브 자료를 조회·추가·변경·삭제한다. 계정 정보와 관리자 승인 설정도 권한에 따라 관리한다. 사용자가 JobSight 또는 연결된 취업 관리 사이트의 데이터를 관리하려 할 때 사용한다.
---
