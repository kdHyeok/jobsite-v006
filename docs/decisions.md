...
## 관리자 요청 본문은 소유자 격리의 명시적 예외 (V16)

사용자가 관리자에게 전달하려고 작성한 버그 제보·기능 제안은 관리자가 읽고 답해야 기능이 성립한다.
따라서 기존의 "관리자는 타인 데이터의 집계 숫자만 본다" 결정을 요청 본문과 피드백에 한해서 확장한다.
기업·공고·직무·이력서 등 기존 업무 데이터의 내용 조회 권한은 그대로 열지 않는다.

## 마지막 직무 삭제는 소속 공고 삭제로 취급한다 (2026-09-16)

기존에는 공고가 직무 없이 남지 않도록 마지막 직무 삭제를 `LAST_POSITION`으로 막았다. 사용자가 직무 화면에서 마지막 항목을 지울 때 공고까지 정리되는 편이 두 화면의 생명주기를 일치시키므로, 소유자 범위에서 공고 행을 잠근 뒤 마지막 직무이면 공고를 삭제하고 DB cascade로 직무를 함께 지운다.
