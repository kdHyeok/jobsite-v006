...
`stage` 의 `관심·작성중·지원완료` 는 내가 정하는 상태고 `코테·면접·결과대기` 는 회사가 정하는 절차다. 절차는 공고마다 달라 고정 enum 에 들어갈 수 없다. `status` 4개 + `recruitment_steps` 로 나눴다. 기존 절차성 값은 `SUBMITTED` + 같은 이름의 step 하나로 옮겼다.
