...
6. **연결 직무는 관계로 저장한다.** `positionIds`는 같은 소유자의 직무만 허용하고 이력서 복제에도 복사한다. 편집 화면은 검색창을 먼저 보여 주고, 검색어가 없으면 아직 선택하지 않은 전체 직무를 최근 수정순으로 보여 준다. 검색 시 이름·기업·공고를 좁히며 선택한 직무는 칩으로 보인다.
...
8. 직무 검색창 아래에 미검색 시 전체 직무가 최근 수정순으로 보이고, 검색해 여러 개 연결한 뒤 선택 칩에서 제거할 수 있는가
